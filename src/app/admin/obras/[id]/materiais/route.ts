...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // 1. Verifica se há stock suficiente no inventário
    ...
    // 2. Cria a transação (Atomic Operation) para garantir que não há erros a meio
    const resultado = await prisma.$transaction([
      // A: Regista a saída na tabela de ligação da obra
      ...
      // B: Desconta o stock no inventário principal
